...
class OptionFlowRule extends base_1.BaseRule {
    async evaluate(context) {
        ...
        if (metrics.bullishScore > 0.7 && pcrChange < 0.8) {
            return this.createResult(true, strength, `Bullish option flow: PCR=${currentPCR.toFixed(2)}, Call buildup at ${metrics.maxCallStrike}`);
        }
        // Check for bearish signals
        if (metrics.bearishScore > 0.7 && pcrChange > 1.2) {
            return this.createResult(true, strength, `Bearish option flow: PCR=${currentPCR.toFixed(2)}, Put buildup at ${metrics.maxPutStrike}`);
        }
        ...
        if (metrics.reversalScore > 0.8) {
            ...
            return this.createResult(true, strength * 0.8, `Potential ${direction} reversal: Significant option unwinding detected`);
        }
        return this.createResult(false, 0, 'No significant option flow signals');
    }
    getRelevantStrikes(context, currentSpot, strikeRange) {
        ...
        const mapped = (state.optionChain || []).map((s) => ({
    ...
    calculateFlowMetrics(flows, currentSpot, minOIChange) {
        let totalCallOI = 0;
        let totalPutOI = 0;
        ...
        const reversalScore = Math.max(callUnwinding / totalCallOI, putUnwinding / totalPutOI);
        return {
            totalCallOI,
            totalPutOI,
            totalCallOIChange,
    ...
    calculateSignalStrength(metrics, currentPCR, pcrChange) {
        ...
        const oiChangeScore = Math.min((Math.abs(metrics.totalCallOIChange) + Math.abs(metrics.totalPutOIChange)) / 50000, 1);
        // Weighted average of all factors
        return Math.min((absoluteScore * 0.4 + pcrScore * 0.2 + volumeScore * 0.2 + oiChangeScore * 0.2), 1);
    }
}
exports.default = OptionFlowRule;
