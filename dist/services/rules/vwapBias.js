...
class VWAPBiasRule extends base_1.BaseRule {
    async evaluate(context) {
        ...
        if (currentClose > upperBand) {
            return this.createResult(true, normalizedStrength, `Bullish VWAP bias: Price ${currentClose.toFixed(2)} above VWAP ${currentVwap.toFixed(2)} + ${deviation.toFixed(2)}`);
        }
        if (currentClose < lowerBand) {
            return this.createResult(true, normalizedStrength, `Bearish VWAP bias: Price ${currentClose.toFixed(2)} below VWAP ${currentVwap.toFixed(2)} - ${deviation.toFixed(2)}`);
        }
        return this.createResult(false, 0, 'Price within VWAP bands');
    }
}
exports.default = VWAPBiasRule;
