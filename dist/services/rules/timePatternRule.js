...
class TimePatternRule extends base_1.BaseRule {
    constructor() {
        super(...arguments);
        this.timeWindows = [
        ...
        switch (currentWindow.expectedBehavior) {
            case 'volatile':
                if (metrics.volatility > volatility_threshold) {
                    ...
                    return this.createResult(true, strength, `High volatility aligned with opening window: ${(metrics.volatility * 100).toFixed(2)}%`);
                }
                break;
            case 'trending':
                if (Math.abs(metrics.trendStrength) > trend_threshold) {
                    ...
                    return this.createResult(true, strength, `${direction} trend in trending window: ${(metrics.trendStrength * 100).toFixed(2)}%`);
                }
                break;
            case 'reversal':
                if (metrics.reversalScore > reversal_threshold) {
                    ...
                    const direction = metrics.trendStrength > 0 ? 'bullish' : 'bearish';
                    return this.createResult(true, strength, `Potential ${direction} reversal in reversal window: Score ${(metrics.reversalScore * 100).toFixed(2)}%`);
                }
                break;
            case 'consolidation':
                if (metrics.volatility < volatility_threshold / 2) {
                    const consolidationQuality = 1 - (metrics.volatility / (volatility_threshold / 2));
                    return this.createResult(true, consolidationQuality, `Strong consolidation in expected window: ${(metrics.volatility * 100).toFixed(2)}% volatility`);
                }
                break;
        }
        return this.createResult(false, 0, `Regular ${currentWindow.expectedBehavior} window activity`);
    }
    getCurrentTimeWindow(time) {
    ...
    calculateTimeWindowMetrics(candles) {
        const returns = [];
        let sumReturns = 0;
...
exports.default = TimePatternRule;
