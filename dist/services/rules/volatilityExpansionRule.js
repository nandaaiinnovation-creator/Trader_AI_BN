...
class VolatilityExpansionRule extends base_1.BaseRule {
    async evaluate(context) {
        ...
        // Calculate ATR
        const atrValues = (0, indicators_1.atr)(highs, lows, closes, atr_period);
        ...
        if (priceChange > 0) {
            return this.createResult(true, normalizedStrength, `Bullish volatility expansion: ATR ${(currentAtr / baselineAtr).toFixed(2)}x baseline`);
        }
        else {
            return this.createResult(true, normalizedStrength, `Bearish volatility expansion: ATR ${(currentAtr / baselineAtr).toFixed(2)}x baseline`);
        }
    }
}
exports.default = VolatilityExpansionRule;
