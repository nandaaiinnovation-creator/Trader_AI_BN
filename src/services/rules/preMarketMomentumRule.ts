...
interface PreMarketStats {
  sgxNiftyChange: number;
  globalMarketSentiment: number;
  sectoralTrend: number;
  volumeRatio: number;
}

export default class PreMarketMomentumRule extends BaseRule {
  async evaluate(context: RuleContext): Promise<RuleResult> {
    ...
    const sentimentScore = this.calculateSentimentScore(
      preMarketStats,
      { sgx_weight, global_weight, sectoral_weight, volume_weight }
    );
    ...
    if (sentimentScore > 0.3 && trendStrength > 0) {
      return this.createResult(
        true,
        signalStrength,
        `Bullish pre-market: SGX ${preMarketStats.sgxNiftyChange.toFixed(1)}%, Volume ${(preMarketStats.volumeRatio).toFixed(1)}x`
      );
    }
    ...
    if (sentimentScore < -0.3 && trendStrength < 0) {
      return this.createResult(
        true,
        signalStrength,
        `Bearish pre-market: SGX ${preMarketStats.sgxNiftyChange.toFixed(1)}%, Volume ${(preMarketStats.volumeRatio).toFixed(1)}x`
      );
    }
    ...
    return this.createResult(
      false,
      0,
      `Mixed pre-market signals: Sentiment=${sentimentScore.toFixed(2)}, Trend=${trendStrength.toFixed(2)}`
    );
  }

  private getPreMarketStats(context: RuleContext): PreMarketStats | null {
  ...
  private calculateSentimentScore(
    ...
    weights: { sgx_weight: number; global_weight: number; sectoral_weight: number; volume_weight: number }
  ): number {
