...
import { findPivots } from '../../utils/indicators';

export default class SwingBreakRule extends BaseRule {
  async evaluate(context: RuleContext): Promise<RuleResult> {
    ...
    const { highs: highPivots, lows: lowPivots } = findPivots(closes, lookback_swing);
    ...
    if (currentClose > lastSwingHigh + buffer) {
      ...
      return this.createResult(
        true,
        Math.min(strength, 1),
        `Bullish swing break: Close ${currentClose.toFixed(2)} above swing high ${lastSwingHigh.toFixed(2)}`
      );
    }
    ...
    if (currentClose < lastSwingLow - buffer) {
      ...
      return this.createResult(
        true,
        Math.min(strength, 1),
        `Bearish swing break: Close ${currentClose.toFixed(2)} below swing low ${lastSwingLow.toFixed(2)}`
      );
    }
