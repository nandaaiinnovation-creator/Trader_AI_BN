...
export default class SectorRotationRule extends BaseRule {
    ...
    if (Math.abs(divergence) > divergence_threshold) {
      ...
      if (divergence > 0) {
        return this.createResult(
          true,
          strength,
          `Bullish rotation: Private banks (${privateMomentum.toFixed(2)}) outperforming PSU banks (${psuMomentum.toFixed(2)})`
        );
      } else {
        return this.createResult(
          true,
          strength,
          `Bearish rotation: PSU banks (${psuMomentum.toFixed(2)}) outperforming Private banks (${privateMomentum.toFixed(2)})`
        );
      }
    }
  ...
  private calculateMomentum(prices: number[], period: number): number {
    const movingAverage = sma(prices, period);
