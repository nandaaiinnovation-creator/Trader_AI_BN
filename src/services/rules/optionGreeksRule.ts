...
export default class OptionGreeksRule extends BaseRule {
  async evaluate(context: RuleContext): Promise<RuleResult> {
    ...
    if (Math.abs(greeks.netDelta) > delta_threshold) {
      ...
      return this.createResult(
        true,
        strength,
        `${direction} delta bias: Net Delta is ${greeks.netDelta.toFixed(0)}`
      );
    }
    ...
    if (greeks.gammaExposure > gamma_threshold) {
        ...
        return this.createResult(
          true,
          strength,
          `High Gamma Exposure: ${greeks.gammaExposure.toFixed(0)}. Potential for sharp moves.`
        );
    }
    ...
    if (Math.abs(greeks.netVega) > vega_threshold) {
        ...
        return this.createResult(
          true,
          strength,
          `Market is net ${direction} volatility: Net Vega is ${greeks.netVega.toFixed(0)}`
        );
    }
  ...
  private aggregateGreeks(optionChain: any[]): AggregatedGreeks {
    let netDelta = 0;
    let netGamma = 0;
    let netVega = 0;
    let netTheta = 0;
