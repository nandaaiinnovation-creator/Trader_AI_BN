...
class DynamicSRRule extends base_1.BaseRule {
    async evaluate(context) {
        ...
        const nearbyLevels = levels.filter(level => Math.abs(level.price - currentPrice) <= proximityThreshold);
        ...
        if (mostSignificantLevel.type === 'resistance' &&
            ...
            return this.createResult(true, Math.min(breakoutStrength, 1), `Breaking resistance: ${mostSignificantLevel.price.toFixed(2)} with ${mostSignificantLevel.touches} touches`);
        }
        if (mostSignificantLevel.type === 'support' &&
            ...
            return this.createResult(true, Math.min(breakoutStrength, 1), `Breaking support: ${mostSignificantLevel.price.toFixed(2)} with ${mostSignificantLevel.touches} touches`);
        }
        ...
        if (nearLevel) {
            ...
            return this.createResult(true, testStrength, `Testing ${nearLevel.type}: ${nearLevel.price.toFixed(2)} (strength: ${(nearLevel.strength * 100).toFixed(1)}%)`);
        }
        return this.createResult(false, 0, `Nearest ${mostSignificantLevel.type}: ${mostSignificantLevel.price.toFixed(2)}`);
    }
    identifySRLevels(candles) {
        ...
        srLevels.forEach(level => {
            ...
            const touches = candles.filter(c => Math.abs(c.high - level.price) / level.price < 0.001 ||
                Math.abs(c.low - level.price) / level.price < 0.001);
            if (touches.length >= 3) {
                const prices = touches.map(c => level.type === 'resistance' ? c.high : c.low);
                const { slope } = (0, indicators_1.linearRegression)(prices, prices.length);
    ...
    calculateLevelStrength(touches, recency) {
