...
class EngulfingVolumeRule extends base_1.BaseRule {
    async evaluate(context) {
        this.validateConfig(['min_volume_mult', 'pivot_window']);
        const { min_volume_mult, pivot_window } = this.config.params;
        const candles = context.candles;
        if (candles.length < pivot_window + 2)
            return this.createResult(false, 0, 'Insufficient data');
        ...
        if (isBullishEngulf && last.volume > avgVol * min_volume_mult)
            return this.createResult(true, 1, 'Bullish engulfing with volume');
        if (isBearishEngulf && last.volume > avgVol * min_volume_mult)
            return this.createResult(true, 1, 'Bearish engulfing with volume');
        return this.createResult(false, 0, 'No engulfing+volume');
    }
}
exports.default = EngulfingVolumeRule;
