...
class HHHLLLLHRule extends base_1.BaseRule {
    async evaluate(context) {
        ...
        const atrValues = (0, indicators_1.atr)(highs, lows, closes);
        const currentAtr = atrValues[atrValues.length - 1];
        const minLegSize = currentAtr * min_leg_size_atr;
        // Find pivot points
        const { highs: highPivots, lows: lowPivots } = (0, indicators_1.findPivots)(closes, pivots_window);
        ...
        if (lastTwoHighs[1] > lastTwoHighs[0] && lastTwoLows[1] > lastTwoLows[0]) {
            ...
            if (highLegSize > minLegSize && lowLegSize > minLegSize) {
                ...
                return this.createResult(true, strength, `Bullish HHHL: HH=${lastTwoHighs[1].toFixed(2)}, HL=${lastTwoLows[1].toFixed(2)}`);
            }
        }
        ...
        if (lastTwoHighs[1] < lastTwoHighs[0] && lastTwoLows[1] < lastTwoLows[0]) {
            ...
            if (highLegSize > minLegSize && lowLegSize > minLegSize) {
                const strength = Math.min((highLegSize + lowLegSize) / (2 * currentAtr), 1);
                return this.createResult(true, strength, `Bearish LLLH: LL=${lastTwoLows[1].toFixed(2)}, LH=${lastTwoHighs[1].toFixed(2)}`);
            }
        }
        return this.createResult(false, 0, 'No valid HH-HL or LL-LH pattern detected');
    }
}
exports.default = HHHLLLLHRule;
