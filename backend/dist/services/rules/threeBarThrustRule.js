...
class ThreeBarThrustRule extends base_1.BaseRule {
    async evaluate(context) {
        this.validateConfig(['thrust_bars', 'min_body_ratio']);
        const { thrust_bars, min_body_ratio } = this.config.params;
        const candles = context.candles;
        if (candles.length < thrust_bars)
            return this.createResult(false, 0, 'Insufficient bars');
        ...
        if (bullish)
            return this.createResult(true, 1, 'Three-bar bullish thrust');
        if (bearish)
            return this.createResult(true, 1, 'Three-bar bearish thrust');
        return this.createResult(false, 0, 'No three-bar thrust');
    }
}
exports.default = ThreeBarThrustRule;
