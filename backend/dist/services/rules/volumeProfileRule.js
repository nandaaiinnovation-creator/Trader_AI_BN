...
class VolumeProfileRule extends base_1.BaseRule {
    async evaluate(context) {
        ...
        const strength = Math.min((volumeStrength * (1 - Math.abs(priceDiff) * 100)) / 2, 1);
        if (Math.abs(priceDiff) < 0.005) { // Within 0.5% of significant level
            if (priceDiff > 0) {
                return this.createResult(true, strength, `Bearish volume node: Heavy volume resistance at ${nearestLevel.price.toFixed(2)} with ${(volumeStrength).toFixed(2)}x threshold volume`);
            }
            else {
                return this.createResult(true, strength, `Bullish volume node: Heavy volume support at ${nearestLevel.price.toFixed(2)} with ${(volumeStrength).toFixed(2)}x threshold volume`);
            }
        }
        return this.createResult(false, 0, 'Price not near significant volume level');
    }
    buildVolumeProfile(candles, levels) {
        if (candles.length === 0)
            return [];
        // Find price range
...
exports.default = VolumeProfileRule;
