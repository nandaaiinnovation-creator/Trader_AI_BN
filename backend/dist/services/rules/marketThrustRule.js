...
class MarketThrustRule extends base_1.BaseRule {
    async evaluate(context) {
        ...
        if (bullishThrusts.length >= min_thrusts) {
            ...
            return this.createResult(true, strength, `Bullish thrust pattern: ${bullishThrusts.length} thrusts, avg ${(avgMagnitude * 100).toFixed(1)}% ATR with ${avgVolume.toFixed(1)}x volume`);
        }
        if (bearishThrusts.length >= min_thrusts) {
            ...
            return this.createResult(true, strength, `Bearish thrust pattern: ${bearishThrusts.length} thrusts, avg ${(avgMagnitude * 100).toFixed(1)}% ATR with ${avgVolume.toFixed(1)}x volume`);
        }
        return this.createResult(false, 0, 'No significant thrust pattern detected');
    }
}
exports.default = MarketThrustRule;
