...
export default class OpeningRangeBreakoutRule extends BaseRule {
  async evaluate(context: RuleContext): Promise<RuleResult> {
    ...
    if (currentCandle.close > openingRange.high + confirmationBuffer) {
      ...
      return this.createResult(
        true,
        strength,
        `Bullish OR breakout: ${currentCandle.close.toFixed(2)} above ${openingRange.high.toFixed(2)} with ${(openingRange.volume / avgVolume).toFixed(1)}x volume`
      );
    }
    ...
    if (currentCandle.close < openingRange.low - confirmationBuffer) {
      ...
      return this.createResult(
        true,
        strength,
        `Bearish OR breakout: ${currentCandle.close.toFixed(2)} below ${openingRange.low.toFixed(2)} with ${(openingRange.volume / avgVolume).toFixed(1)}x volume`
      );
    }
  ...
  private calculateOpeningRange(candles: any[], marketOpen: Date, rangeMinutes: number): OpeningRange | null {
    const rangeEndTime = new Date(marketOpen.getTime() + rangeMinutes * 60000);
    let high = -Infinity;
    let low = Infinity;
    let volume = 0;
    let firstCandleClose: number | null = null;

    for (const candle of candles) {
      const candleTime = new Date(candle.timestamp);
      if (candleTime < marketOpen) continue;
      if (candleTime > rangeEndTime) break;
