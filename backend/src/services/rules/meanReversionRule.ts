...
import { atr } from '../../utils/indicators/atr';

export default class MeanReversionRule extends BaseRule {
  ...
  const _recentCloses = closes.slice(-mean_revert_bars);
  void _recentCloses;
    ...
    if (currentClose < currentLower) {
      ...
      if (currentClose <= lowestLow) {
        return this.createResult(
          true,
          normalizedStrength,
          `Bullish mean reversion: Price ${(distanceFromMean * 100).toFixed(2)}% below mean with ${(expectedVolatility * 100).toFixed(2)}% volatility`
        );
      }
    }
    ...
    if (currentClose > currentUpper) {
      ...
      if (currentClose >= highestHigh) {
        return this.createResult(
          true,
          normalizedStrength,
          `Bearish mean reversion: Price ${(distanceFromMean * 100).toFixed(2)}% above mean with ${(expectedVolatility * 100).toFixed(2)}% volatility`
        );
      }
    }
