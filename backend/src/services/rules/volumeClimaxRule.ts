...
export default class VolumeClimaxRule extends BaseRule {
  async evaluate(context: RuleContext): Promise<RuleResult> {
    ...
    const volumeSMA = sma(volumes, volume_ma_period);
    ...
    if (priceChange < -price_reversal_pct) {
      return this.createResult(
        true,
        normalizedStrength,
        `Bullish volume climax: ${volumeRatio.toFixed(2)}x average volume with ${Math.abs(priceChange).toFixed(2)}% price drop`
      );
    }
    ...
    if (priceChange > price_reversal_pct) {
      return this.createResult(
        true,
        normalizedStrength,
        `Bearish volume climax: ${volumeRatio.toFixed(2)}x average volume with ${priceChange.toFixed(2)}% price surge`
      );
    }
