...
export default class InsideBarRule extends BaseRule {
  async evaluate(context: RuleContext): Promise<RuleResult> {
    ...
    if (candles.length < lookback + 2) return this.createResult(false, 0, 'Insufficient history for inside-bar');
    ...
    if (current.high <= prev.high && current.low >= prev.low) {
      ...
      if (breakoutBull) return this.createResult(true, 1, 'Inside-bar bullish breakout');
      if (breakoutBear) return this.createResult(true, 1, 'Inside-bar bearish breakout');
