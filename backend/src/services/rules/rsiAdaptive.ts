...
export default class RSIAdaptiveRule extends BaseRule {
  async evaluate(context: RuleContext): Promise<RuleResult> {
    ...
    const rsiValues = rsi(closes, rsi_len);
    ...
    const normalizedStrength = (val: number, threshold: number) => 
      Math.min(Math.abs(val - threshold) / 10, 1); // 10 points from threshold = full strength

    if (currentRsi > adjustedUpper && !recentBearish) {
      return this.createResult(
        true,
        normalizedStrength(currentRsi, adjustedUpper),
        `Bearish RSI: ${currentRsi.toFixed(2)} above ${adjustedUpper} in ${context.regime} regime`
      );
    }
    ...
    if (currentRsi < adjustedLower && !recentBullish) {
      return this.createResult(
        true,
        normalizedStrength(currentRsi, adjustedLower),
        `Bullish RSI: ${currentRsi.toFixed(2)} below ${adjustedLower} in ${context.regime} regime`
      );
    }
